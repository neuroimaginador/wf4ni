...
correct_slice_time <- function(bold) {
  ...
  bold_corrected <- fslr::fslslicetimer(file = my_bold)
...
skull_strip <- function(image) {
  ...
  res <- fslr::fslbet(infile = my_file, opts = "-m")
...
normalize_data <- function(t1_ss, bold_ss) {
  ...
  res <- fslr::fslsplit(infile = my_bold, direction = "t")
  ...
  tmp <- fslr::flirt(infile = my_bold1, 
                     reffile = my_t1, 
                     omat = func2t1_mat, 
  ...
  t12std_img <- fslr::flirt(infile = my_t1, 
                     reffile = flirtref, 
  ...
  #apply the transform
  func2std_img <- fslr::flirt_apply(infile = my_bold,
                                    reffile = flirtref,
                                    initmat = func2standard_mat,
                                    opts = " -interp trilinear")
  
  return(list(t12std_img, func2std_img))
...
filter_func <- function(func, tr) {
  ...
  library(ANTsR)
  func_mat <- frequencyFilterfMRI(func_mat, tr)
...
```{r correlation}
# do the parcellation
extract_time_series <- function(bold) {
  ...
  basedir <- "./vignettes/resources/rsfMRI/"
  
  corr_label <- file.path(basedir, 'data', 'aal_MNI_V4.nii')
  corr_text <- file.path(basedir, 'data', 'aal_MNI_V4.txt')
